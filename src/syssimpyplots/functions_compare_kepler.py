...
def compute_distances_sim_Kepler(sss_per_sys, sss, ssk_per_sys, ssk, weights, dists_include, N_sim, cos_factor=1., AD_mod=True, print_dists=True, compute_ratios=gen.compute_ratios_adjacent):
    ...
    dists['duration_ratios_KS'] = KS_dist(np.log10(sss['xi_obs']), np.log10(ssk['xi_obs']))[0]
    dists['duration_ratios_nonmmr_KS'] = KS_dist(np.log10(sss['xi_nonres_obs']), np.log10(ssk['xi_nonres_obs']))[0]
    dists['duration_ratios_mmr_KS'] = KS_dist(np.log10(sss['xi_res_obs']), np.log10(ssk['xi_res_obs']))[0]
