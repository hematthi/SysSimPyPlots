...
savefigures = False
savefigures_directory = '/Users/hematthi/Documents/GradSchool/Research/SysSim/Figures/Hybrid_NR20_AMD_model1/clustered_initial_masses/Observed/' + 'Radius_valley_measures/Fit_some8p1_KS_params9/' #catalog62_repeated/'
...
for i in range(1,runs+1):
    ...
    radii_delta_i = radii_delta_from_period_radius_gap(sss_i_radii, sss_i_periods, m=-0.10, Rgap0=2.40)
...
depth_binned_Kep = measure_and_plot_radius_valley_depth_using_global_bins(ssk_radii, x_min=radii_min, x_max=radii_max, plot_fig=True, save_name=save_dir_Kepler + 'Kepler_radius_valley_depth_binned.pdf', save_fig=savefigures_Kepler)
depth_kde_Kep = measure_and_plot_radius_valley_depth_using_kde(ssk_radii, x_min=radii_min, x_max=radii_max, bw_scotts_factor=bw_factor, plot_fig=True, save_name=save_dir_Kepler + 'Kepler_radius_valley_depth_kde.pdf', save_fig=savefigures_Kepler)

radii_delta_Kep = radii_delta_from_period_radius_gap(ssk_radii, ssk_periods, m=-0.10, Rgap0=2.40)
radii_delta_depth_kde_Kep = measure_and_plot_radius_valley_depth_using_kde(radii_delta_Kep, radius_valley_bounds=(-0.2,0.2), x_min=radii_min-2., x_max=radii_max-1.5, bw_scotts_factor=bw_factor, xlabel_text=r'Gap subtracted radius, $R_p - R_{\rm gap}$ [$R_\oplus$]', plot_fig=True, save_name=save_dir_Kepler + 'Kepler_radius_delta_valley_depth_kde.pdf', save_fig=savefigures_Kepler)
...
for i in iSort[:10]:
    ...
    if sort_by == 'depth_binned':
        ...
        radii_delta = radii_delta_from_period_radius_gap(sss_radii_all[i], sss_periods_all[i], m=-0.10, Rgap0=2.40)
        depth = measure_and_plot_radius_valley_depth_using_kde(radii_delta, radius_valley_bounds=(-0.2,0.2), x_min=radii_min-2., x_max=radii_max-1.5, bw_scotts_factor=bw_factor, xlabel_text=r'Gap subtracted radius, $R_p - R_{\rm gap}$ [$R_\oplus$]', plot_fig=True, save_name=save_name, save_fig=savefigures)
    else:
...
plot_fig_pdf_simple([sss_radii_all[i] for i in iSort[:N_plot]] + [ssk_radii], [], x_min=radii_min, x_max=radii_max, c_sim=[cmap.to_rgba(i) for i in np.linspace(0.1,0.6,N_plot)] + ['k'], ls_sim=['-']*N_plot + ['-'], lw=[1.]*N_plot + [2.], labels_sim=['Simulated catalogs \n''with largest valleys'] + [None]*(N_plot-1) + ['Kepler'], xlabel_text=r'Planet radius, $R_p$ [$R_\oplus$]', afs=afs, tfs=tfs, lfs=lfs, legend=True, fig_size=(10,6), fig_lbrt=[0.15,0.15,0.95,0.95]) # ls_sim=[(0,(1,1))]*N_plot + ['-']
if savefigures:
